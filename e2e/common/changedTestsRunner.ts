...
export class ChangedTestsRunner {
  ...
  private static async fetchMasterBranch(): Promise<void> {
    try {
      ...
      await this.git.fetch(
        `https://github.com/hmcts/nfdiv-manage-case-e2e-tests.git`,
        "master",
      );
  ...
  private static async runPlaywrightTests(testFiles: string[]): Promise<void> {
    ...
    try {
      const command: string = `yarn playwright test ${testFiles.join(" ")} --config=playwright.service.config.ts --project chrome`;
